...
print("Colonnes du DataFrame df:", df.columns)
...
print('Accuracy: %.2f' % (accuracy*100))
print("Colonnes de X pour vérifier les index:", X.columns)


# Scatter plot and regression line
feature_index = 1 # Choose the first characteristic for visualization
feature_name = X.columns[feature_index] # and get characteristic name

# sort the data
sorted_indices = np.argsort(X_test.iloc[:, feature_index])
sorted_feature_values = X_test.iloc[sorted_indices, feature_index]
sorted_predictions = model.predict(X_test.iloc[sorted_indices])
